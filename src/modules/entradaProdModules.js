...
class entradaProdDbModules {
  async createEntradaProd(nome, lote, dataValidade) {
    try {
  ...
  async getEntradaProdutoById(id) {
    try {
      const entradaProduto = dbConnect("entradaProd").where("id", id).first();
  ...
  async updateEntradaProduto(id, nome, lote, dataValidade) {
    try {
      const date = new Date();
      const updatedEntradaProduto = await dbConnect("entradaProd").where("id", id).update(
        {
          nome: nome,
          lote: lote,
        ...
        ["id", "nome", "lote", "dataValidade", "dataEntrada","created_at", "updated_at"]
      );
  ...
  async getEntradaProdutos() {
    try {
      const entradaprodutos = await dbConnect("entradaProd").select().table("entradaProd");
